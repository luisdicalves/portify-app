...
- **Deploy readiness: the migration is live in production.** It was applied
  to and fully validated against `portify` on 2026-07-10 (see the runbook's
  "Production validation log" for the complete result: schema/RLS/
  constraints all match, `database.types.ts` regenerated byte-identical, a
  real end-to-end XTB import including `withholding_tax`/`deposit`/
  `interest_tax` confirmed in the database, RLS confirmed with two real
  users). **Correction, 2026-07-11:** `portify` was originally believed to
  be staging — the project owner had confirmed it as such — but this was
  mistaken; `portify` is production, and this organization currently has
  **no separate staging project** (`list_projects` shows only `portify` and
  an unrelated `INACTIVE` project, `portifyv1`). All disposable test data
  created during validation was deleted from production once this was
  caught. See
  [import-audit-migration-runbook.md](import-audit-migration-runbook.md) for
  the full apply/verify/rollback history and
  [release-checklist.md](release-checklist.md#import-audit-log-release-checklist)
  for the final checklist state. Imports fail closed (abort, nothing
  written, friendly error) in any environment where this migration hasn't
