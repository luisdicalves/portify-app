...
const OPTIONS = [
  { id: 'conservative', label: 'Conservador', desc: 'Prefiro proteger o capital.', icon: 'shield', color: '#047a3d' },
  { id: 'moderate', label: 'Moderado', desc: 'Equilíbrio entre risco e retorno.', icon: 'balance', color: '#f59e0b' },
  { id: 'aggressive', label: 'Agressivo', desc: 'Aceito volatilidade por mais retorno.', icon: 'local_fire_department', color: '#c4302b' },
];
...
export default function RiskPage() {
  ...
  return (
    <div className="phone-shell" style={{ justifyContent: 'space-between' }}>
      <StepHeader step={3} total={6} back={() => router.back()} title="Perfil de risco" sub="Define quanto risco está disposto a aceitar." />
          ...
          return (
            <button key={o.id} onClick={() => setSelected(o.id)} style={{
              ...
              <div style={{ width: 48, height: 48, borderRadius: 'var(--radius-md)', background: on ? 'var(--primary-strong)' : 'var(--surface-high)', display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none' }}>
                <span className={`material-symbols-outlined${on ? ' icf' : ''}`} style={{ fontSize: 24, color: on ? '#fff' : o.color }}>{o.icon}</span>
              </div>
              <div style={{ flex: 1 }}>
      ...
      <div style={{ padding: '24px 24px 48px' }}>
        <button className="btn-primary" onClick={() => router.push('/auth/objective')}>Continuar</button>
      </div>
    </div>
  );
}
