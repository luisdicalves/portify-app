...
function getWarning(years: number): { text: string; color: string } | null {
  ...
  if (years < 5) return { text: 'Com menos de 5 anos, prefere ativos estáveis como ETFs de obrigações.', color: '#F59E0B' };
  if (years >= 20) return { text: 'Excelente! Com 20+ anos podes aceitar mais risco e beneficiar do crescimento composto.', color: 'var(--gain)' };
  return null;
}
  ...
  return (
    <div className="phone-shell" style={{ overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '24px 20px 0' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: '32px 0' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
            <input
              ...
              value={years} onChange={e => handleChange(e.target.value)}
              placeholder="—"
              style={{
                width: 120, fontSize: 56, fontWeight: 700, textAlign: 'center',
                ...
                letterSpacing: '-0.02em', MozAppearance: 'textfield',
              }}
            />
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
            {[3, 5, 10, 15, 20, 30].map(y => (
              <button key={y} onClick={() => { setYears(String(y)); setError(''); }} style={{
        ...
        {warning && (
          <div style={{ background: 'var(--surface-low)', border: `1px solid ${warning.color}`, borderRadius: 'var(--radius-lg)', padding: '12px 16px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
