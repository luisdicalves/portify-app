...
export default function FinancialPage() {
  ...
  async function handleContinue() {
    ...
    if (user) {
      await supabase.from('profiles').update({ financial_status: OPTIONS[selected].id }).eq('id', user.id);
    }
  ...
  return (
    <div className="phone-shell" style={{ overflow: 'hidden' }}>
      <StepHeader step={6} total={9} back={() => router.back()} title="Situação financeira" sub="Como descreves a tua situação financeira atual?" />
